...
struct ParksController<Context: RequestContext> {
  ...
  func index(_ request: Request, context: Context) async throws -> [Park] {
    ...
    // Logging
    logger.info(
      "Handling \(request.uri.string) request",
      metadata: [
        "operation method": "\(request.method.rawValue)",
        "number of parks": "\(parks.count)",
      ]
    )

    // Metrics
    let counter = Counter(label: "parks_of_prague_index_counter")
    counter.increment()

    return try await withSpan("parks_of_prague_index_query") { span in
      try await client.withConnection { conn in
        ...
        for try await (id, latitude, longitude, details) in stream.decode((UUID, Float, Float, OracleJSON<Park.Details>).self) {
          parks.append(.init(id: id, coordinates: Park.Coordinates.init(latitude: latitude, longitude: longitude), details: Park.Details.init(name: details.value.name)))
        }
      }
      if let traceID = span.context.spanContext?.traceID {
  ...
  func show(_ request: Request, context: Context) async throws -> Park? {
    ...
    return try await client.withConnection { conn in
      ...
      for try await (id, latitude, longitude, details) in stream.decode((UUID, Float, Float, OracleJSON<Park.Details>).self) {
        return Park(id: id, coordinates: Park.Coordinates.init(latitude: latitude, longitude: longitude), details: Park.Details.init(name: details.value.name))
      }
